...
public class VoiceManager {
     ...
     * 开始录音（外部调）
     *
     * @param recordFileDir 音频存放文件夹
     */
    public void startVoiceRecord(String recordFileDir, String recordFilePath) {
        if (!isSDCardAvailable()) return;
        this.recordFileDir = recordFileDir;
        this.recordFilePath = recordFilePath;

        if (recordFileDir == null) {
            this.recordFileDir = getDirName(recordFilePath);
        }
        ...
        try {
            ...
            if (VoiceTimeUtils.timeSpanSecond(mRecTimeSum).mSpanSecond == 0) {
                ...
                if (file.length() > 0) {
                    ...
                    //完成录音
                    if (voiceRecordCallBack != null) {
                        voiceRecordCallBack.recFinish(mRecTimeSum,
                                String.format(Locale.CHINA, "%02d:%02d:%02d", ts.mSpanHour, ts.mSpanMinute, ts.mSpanSecond), file.getAbsolutePath());
                    }
                }
            }
...
//                        });
    ...
    private File getOutputVoiceFile(ArrayList<File> list) {
        ...
        File recDirFile = recAudioDir(recordFileDir);

        // 创建音频文件,合并的文件放这里
        // File resFile = new File(recDirFile, mMinute1 + ".amr");
        File resFile;
        if (recordFilePath == null) {
            resFile = new File(recDirFile, mMinute1 + ".mp3");
        } else {
            resFile = new File(recordFilePath);
        }
    ...
    private File prepareRecorder(MediaRecorder mr, boolean start) {
        ...
        try {
            String path = recAudioDir(recordFileDir).getAbsolutePath();
