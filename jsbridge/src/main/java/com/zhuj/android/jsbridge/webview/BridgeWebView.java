...
public class BridgeWebView extends WebView {
    ...
    private void init() {
        ...
        super.setWebChromeClient(mWebChromeClient); // webChromeClient，是本类覆写这个方法设置的WebChromeClient
        super.addJavascriptInterface(innerJavascriptInterface, BRIDGE_NAME);
        addInternalJavascriptObject();
    }

    /**
     * 供 JavaScript 调用 Java 的方法
     * <p>
     * only public methods that are annotated with JavascriptInterface can be accessed from JavaScript.
     * injected objects will not appear in JavaScript until the page is next (re)loaded.
     * Build.VERSION_CODES.LOLLIPOP and above, methods of injected Java objects are enumerable from JavaScript.
     */
    private class InnerJavascriptInterface {
        ...
        private void PrintDebugInfo(String error) {
            Log.d(TAG, error);
            if (isDebug) {
                evaluateJavascript(String.format("alert('%s')", "DEBUG error msg:\\n" + error.replaceAll("\\'", "\\\\'")));
            }
        }
        ...
        public String call(String methodName, String argStr) {
            ...
            try {
                ...
                if (args.has("_jscbmn")) {
                    callback = args.getString("_jscbmn"); // 参数中传递了JavaScript异步执行的函数名
                }
                if (args.has("data")) {
                    arg = args.get("data");
                }
            ...
            boolean asyn = false;
            try {
                method = cls.getMethod(methodName, Object.class, OnCallJavaCallback.class);
                asyn = true;
            } catch (Exception e) {
                try {
            ...
            try {
                if (asyn) {
                    ...
                    method.invoke(jsb, arg, new OnCallJavaCallback() {

                        @Override
                        public void onCompleted(Object retValue) {
                        ...
                        private void complete(Object retValue, boolean complete) {
                            try {
                                ...
                                if (cb != null) {
                                    ...
                                    if (complete) {
                                        script += "delete window." + cb; // 这个callback放到window对象，完成时删除
                                    }
    ...
    private void addInternalJavascriptObject() {
        addJavascriptObject("_dsb", new Object() {
            @Keep
            @JavascriptInterface
            public boolean hasNativeMethod(Object args) throws JSONException {
                ...
                if (jsb == null) return false;

                Class<?> cls = jsb.getClass();
                boolean asyn = false;
                Method method = null;
                try {
                    method = cls.getMethod(nameStr[1], Object.class, OnCallJavaCallback.class);
                    asyn = true;
                } catch (Exception e) {
                    try {
            ...
            public void dsinit(Object jsonObject) {
                BridgeWebView.this.dispatchStartupQueue(); // ds 初始化完成，清空Java 调用 JavaScript 的调用列表
            }

            @Keep
            @JavascriptInterface
            public void returnValue(final Object obj) { // 这里处理Java 调用 JavaScript 的返回值
                runOnMainThread(new Runnable() {
                    @Override
                    public void run() {
     ...
     */
    public void addJavascriptObject(String namespace, Object object) {
        if (namespace == null) {
            namespace = "";
        }
        if (object != null) {
