...
public final class FileUtils {
    ...
    public String getFileExtensionName(String filepath) {
    ...
    // public static List<File> findFile(File parentFile, FilenameFilter filenameFilter, int depth) {
    ...
    //     }
    //     if (depth == 1) {
    //         return Arrays.asList(parentFile.listFiles(filenameFilter));
    //     } else {
    //         List<File> arrayFiles = new ArrayList<>();
    //         File[] list = parentFile.listFiles(filenameFilter);
    //         if (list != null && list.length != 0) {
    //             arrayFiles.addAll(Arrays.asList(list));
    //         }
    //         for (File file : files) {
    //             arrayFiles.addAll( findFile(file);
    //         }
    //         return arrayFiles.toArray(new File[0]);
    //     }
    //
    // }
    ...
    public static String getDiskCacheDir() {
        return isSDCardExist() && Androids.getApp().getExternalCacheDir() != null ? Androids.getApp().getExternalCacheDir().getPath() : Androids.getApp().getCacheDir().getPath();
    }
    ...
    public static String getDiskFilesDir() {
        File file = Androids.getApp().getExternalFilesDir(null);
        return isSDCardExist() && file != null ? file.getPath() : Androids.getApp().getFilesDir().getPath();
    }
    ...
    public static String getDiskDir(String fileDir) {
        if (isSDCardExist()) {
            ...
            return Androids.getApp().getDir(fileDir, Context.MODE_PRIVATE).getPath();
        }
    }
    ...
    public static String getDiskDir() {
        return getDiskDir(Androids.getApp().getPackageName());
    }
    ...
    private static void closeIO(Closeable dis) {
        try {
            if(dis != null){
                dis.close();
            }
